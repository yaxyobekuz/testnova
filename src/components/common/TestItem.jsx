...
const TestItem = ({ title, description, icon, count, _id: id }) => {
  return (
    <li className="test-item group">
      <Icon size={56} src={icon} alt={title} className="size-14 mb-5" />
      ...
      <Icon
        ...
        className="absolute top-8 right-8 size-8 opacity-0 translate-y-1.5 -translate-x-1.5 transition-all duration-200 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 peer-focus:opacity-100 peer-focus:translate-y-0 peer-focus:translate-x-0"
      />
    </li>
  );
};
